...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if(p){document.getElementById("searchInput").value=p;searchFoundItems();}else loadFoundItems();
});

async function loadFoundItems() {
  showLoading("foundItemsContainer");
  try { renderFoundItems(await(await fetch(`${API_BASE}/found-items`)).json()); }
  catch { showError("foundItemsContainer","Could not load items. Is the backend running?"); }
}
...
async function searchFoundItems() {
  ...
  showLoading("foundItemsContainer");
  try{renderFoundItems(await(await fetch(`${API_BASE}/found-items/search?${p}`)).json(),true);}
  catch{showError("foundItemsContainer","Search failed.");}
}

function resetFoundFilters(){
...
function renderFoundItems(items,isSearch=false){
  const info=document.getElementById("resultsInfo");
  if(info)info.textContent=items.length?`${items.length} result${items.length!==1?"s":""}`:""
  if(!items||!items.length){showEmpty("foundItemsContainer",isSearch?"No items match your search":"No found items yet",isSearch?"Try different filters.":"Check back soon.");return;}
  document.getElementById("foundItemsContainer").innerHTML=items.map(i=>`
    <div class="col-xl-3 col-lg-4 col-md-6">
      <div class="card h-100" style="cursor:pointer;" onclick="window.location='item-details.html?id=${i.id}&type=FOUND'">
        ...
        <div class="card-body d-flex flex-column" style="padding:15px 17px;">
          ...
          <div style="font-size:.75rem;display:flex;flex-direction:column;gap:3px;margin-bottom:13px;">
            <span style="display:flex;align-items:center;gap:4px;color:var(--text-3);">${PIN_SVG} ${escHtml(i.location||"—")}</span>
            <span style="display:flex;align-items:center;gap:4px;color:var(--text-4);">${CAL_SVG} ${i.date||"—"}</span>
          </div>
          <a href="item-details.html?id=${i.id}&type=FOUND" class="btn-uni-primary btn btn-sm w-100" style="justify-content:center;" onclick="event.stopPropagation()">View Details</a>
        </div>
      </div>
    </div>`).join("");
}
